package com.misy.mybatis.realm;
...
public class MyShiroRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        ...
        // 从数据库获取对应用户名密码的用户
        UserAccount userList = userService.getUser(user);
        if (userList != null) {
            // 用户为禁用状态
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (principal instanceof UserAccount) {
            UserAccount userLogin = (UserAccount) principal;
//            Set<String> roles = roleService.findRoleNameByUserId(userLogin.getUserId());
//            authorizationInfo.addRoles(roles);

//            Set<String> permissions = userService.findPermissionsByUserId(userLogin.getUserId());
